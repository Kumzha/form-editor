  ...
  (
    ...
    const normalizeContentEditable = (element: HTMLElement | null) => {
      ...
      if (isSafari()) {
        ...
        const hasNonTextNodes = childNodes.some(
    ...
    const handleTextInput = () => {
      ...
      if (selection && selection.rangeCount > 0) {
        ...
        if (range.startContainer.nodeType === Node.TEXT_NODE) {
          ...
          if (
            contentEditableRef.current &&
            range.startContainer !== contentEditableRef.current.firstChild
          ) {
    ...
    const handleKeyDown = (e: React.KeyboardEvent<HTMLDivElement>) => {
      if (e.key === "Enter") {
        ...
        if (textNode.nodeType === Node.TEXT_NODE) {
          ...
          // Update the text node with newline
    ...
    useEffect(() => {
      if (
        contentEditableRef.current &&
        plainText !== contentEditableRef.current.textContent
      ) {
        contentEditableRef.current.textContent = plainText;
      }
    }, [plainText]);
              ...
              if (hadFocus) {
                ...
                try {
                  if (selection) {
                    ...
                    const newPosition = Math.min(
                      cursorPosition,
                      (contentEditableRef.current.textContent || "").length
                    );
