...
const UploadExamples: React.FC<{ isOpen: boolean }> = ({ isOpen }) => {
  ...
  const onDrop = useCallback((acceptedFiles: File[]) => {
    setFiles((prevFiles) => {
      ...
      if (totalSize > MAX_TOTAL_SIZE) {
        toast.error("Total file size exceeds 10MB limit. Some files were not added.")
        return prevFiles
      }
      return newFiles
    })
  ...
  const uploadFileMutation = useMutation({
    ...
    onError: (error, variables) => {
      toast.error(`Failed to upload ${variables.name}: ${error instanceof Error ? error.message : "Unknown error"}`)
    },
  })

  // Add this mutation after the uploadFileMutation
  const deleteFileMutation = useMutation({
    mutationFn: async ({ formName, formId, fileName }: { formName: string; formId: string; fileName: string }) => {
      const authToken = localStorage.getItem("authToken")
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["documents", selectedForm?.form_id] })
  ...
  const batchUploadMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      if (successfulFiles.length === files.length) {
        toast.success(`All ${files.length} files uploaded successfully as ${documentType} documents.`)
        setFiles([])
      } else if (successfulFiles.length > 0) {
        toast.success(`${successfulFiles.length} of ${files.length} files uploaded successfully.`)
        // Keep only the failed files
        const successFileNames = new Set(successfulFiles.map((file) => file.name))
        setFiles((prevFiles) => prevFiles.filter((file) => !successFileNames.has(file.name)))
      }
  ...
  const isUploading = batchUploadMutation.isPending || uploadFileMutation.isPending

  return (
    <Dialog>
      <DialogTrigger asChild>
        <SidebarItem text="Upload Examples" logo={<MdOutlineDriveFolderUpload size={20} />} isOpen={isOpen} />
      </DialogTrigger>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Upload Examples</DialogTitle>
          <DialogDescription>
            Upload selected .pdf or .docx files to increase the accuracy and the style of content generation.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="mb-4">
          ...
          {selectedForm ? (
            ...
            <p className="text-sm text-red-500">No form selected. Please select a form first.</p>
          )}
        </div>
        ...
        >
          ...
          <p className="text-sm text-gray-500 mt-2">Only .pdf and .docx files are allowed</p>
        </div>
        {files.length > 0 && (
          <div className="mt-4">
            ...
            <p className="text-sm text-gray-500 mt-2">Total size: {bytes(totalSize)} / 10MB</p>
          </div>
        )}
        {selectedForm?.uploaded_files && selectedForm.uploaded_files.length > 0 && (
          <div className="mt-4">
